...
use chrono::Utc;
use orderbook::{
    primitives::{MatchedOrderVerbose, SingleSwap},
    traits::Orderbook,
    OrderbookProvider,
};
...
use crate::htlc::{bigdecimal_to_u256, compute_order_id, parse_address, parse_secret_hash, ERC20HTLC, NativeHTLC};
use crate::orders::PendingOrdersProvider;
use crate::settings::{Erc20Pair, Settings};

const ZERO_ADDRESS: &str = "0x0000000000000000000000000000000000000000";

pub struct RefunderService<P> {
    provider: Arc<P>,
...
impl<P: Provider + Clone + Send + Sync + 'static> RefunderService<P> {
    pub fn new(
        provider: Arc<P>,
        orderbook: Arc<OrderbookProvider>,
        orders_provider: PendingOrdersProvider,
    ...
    pub async fn run(&self) {
        info!(chain = %self.chain_name, "RefunderService started");
        loop {
            if let Err(e) = self.process().await {
                error!(chain = %self.chain_name, "RefunderService error: {e}");
            }
    ...
    async fn process(&self) -> eyre::Result<()> {
        ...
        let current_block_bd = BigDecimal::from(current_block);

        for order in &orders {
            let action_info = match self.order_mapper.map(order, Some(&current_block_bd)).await {
                Ok(info) => info,
                Err(e) => {
            ...
            match action_info.action {
                HTLCAction::Refund => {
                    if let Some(swap) = action_info.swap {
                        if let Err(e) = self.refund_swap(order, &swap).await {
                            error!(order_id = %order.create_order.create_id, "Failed to refund swap: {e}");
                        }
                    }
                }
                _ => {}
    ...
    async fn refund_swap(&self, order: &MatchedOrderVerbose, swap: &SingleSwap) -> eyre::Result<()> {
        let initiator = parse_address(&swap.initiator)?;
        let redeemer = parse_address(&swap.redeemer)?;
        let timelock = U256::from(swap.timelock as u64);
        let amount = bigdecimal_to_u256(&swap.filled_amount)?;
        let secret_hash = parse_secret_hash(&swap.secret_hash)?;
        ...
        if token_addr == ZERO_ADDRESS || token_addr == "primary" {
            ...
            let receipt = contract.refund(order_id).send().await?.get_receipt().await?;
        ...
        self.orderbook
            .update_swap_refund(&swap.swap_id, &tx_hash, block_number, Utc::now())
            .await?;
