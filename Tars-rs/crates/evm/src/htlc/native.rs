...
#[async_trait]
impl HTLCInterface for NativeHTLCContract {
    ...
    async fn initiate_with_user_signature_calldata(
        ...
        Err(HTLCError::UnsupportedAction {
            action: "initiate with signature for native htlc".to_string(),
        })
    }
    ...
    async fn instant_refund_calldata(
        ...
        let data = self
            .instantRefund(swap.order_id, signature)
            .calldata()
            .clone();
    ...
    async fn get_typed_data(
        ...
        Err(HTLCError::UnsupportedAction {
            action: "get_typed_data not supported for native htlc".to_string(),
        })
    }
