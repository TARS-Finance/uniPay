...
#[async_trait]
impl HTLCInterface for NativeHTLCv3Contract {
    ...
    async fn initiate_with_user_signature_calldata(
        ...
        Err(HTLCError::UnsupportedAction {
            action: "initiate with user signature for native htlc v3".to_string(),
        })
    }
    ...
    async fn get_typed_data(
        ...
        Err(HTLCError::UnsupportedAction {
            action: "get_typed_data not supported for native htlc v3".to_string(),
        })
    }
